document.addEventListener("DOMContentLoaded", () => {
  const userId = "1376022909096689668"; //replace your discord id here
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      console.log("API Response: - discord.js:15", data);
      ...
      if (data.avatar_decoration && data.avatar_decoration.asset) {
        ...
        console.warn("No avatar frame asset found. - discord.js:23");
        avatarFrame.style.display = "none";
      }
    })
    .catch((error) => {
      console.error("Error fetching user data: - discord.js:28", error);
    });
});
